(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var navChat = $('#navChat'), navDes = $('#navDesigner'), chatView = $('#chatView'), desView = $('#designerView'), chatSide = $('#chatSide'), desSide = $('#designerSide');
    function showChat(){ if(!navChat) return; navChat.classList.add('on'); navDes.classList.remove('on'); chatView.style.display=''; desView.style.display='none'; chatSide.style.display=''; desSide.style.display='none'; }
    function showDes(){ navDes.classList.add('on'); navChat.classList.remove('on'); chatView.style.display='none'; desView.style.display=''; chatSide.style.display='none'; desSide.style.display=''; desRender(); }
    if(navChat && navDes){ navChat.addEventListener('click', showChat); navDes.addEventListener('click', showDes); }
    desInit();
  });

  // ---- Phone Designer (very user-friendly, offline, supports other devices) ----
  ...
  var desEls = load('orbit_designs', null) ? load('orbit_designs', []) : [];
  var desSel = null;
  var desDev = load('orbit_device', 'phone');
  function desSave(){ save('orbit_designs', desEls); save('orbit_device', desDev); }
  ...
  function desRender(){
    ...
    desEls.forEach(function(el){
      ...
      if(el.type==='text'){ e.textContent=el.text; e.style.background='transparent'; e.style.justifyContent='center'; }
  ...
  function desInit(){
    var pal=document.querySelectorAll('#palette .pBtn');
    for(var i=0;i<pal.length;i++) pal[i].addEventListener('click', function(){ desAdd(this.dataset.type); });
    var devs=document.querySelectorAll('#deviceList .dBtn');
    for(var i=0;i<devs.length;i++) devs[i].addEventListener('click', function(){ desSetDevice(this.dataset.dev); });
    var tmps=document.querySelectorAll('#tmplList .tBtn');
    for(var i=0;i<tmps.length;i++) tmps[i].addEventListener('click', function(){ desLoadTemplate(this.dataset.tmpl); });
    ...
    var ex=$('#exportDesign'); if(ex) ex.addEventListener('click', function(){ var blob=new Blob([JSON.stringify({device:desDev,elements:desEls},null,2)],{type:'application/json'}); var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='orbit-design.json'; a.click(); });
